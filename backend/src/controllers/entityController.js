...
export async function create(req, res) {
    ...
    const newEntity = await model.createEntity(name, surname, email, description);
    res.status(201).json(newEntity);
}

export async function update(req, res) {
    const { name, surname, email, description } = req.body;
    const updated = await model.updateEntity(req.params.id, name, surname, email, description);
    res.json(updated);
}
